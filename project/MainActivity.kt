            ...
            MyAppTheme {
                ...
                NavHost(navController = navController, startDestination = "conversation") {
                    composable("conversation") {
                        ConversationScreen(navController = navController, userDao = userDao, messageDao = messageDao)
                    }
                    ...
                    composable("camera") {
                        CameraScreen(navController = navController, photoDao = photoDao, messageDao = messageDao, userDao = userDao)
                    }
                }
            }
        }
...
data class Message(val author: String, val body: String?, val profileImage: String?, val imagePath: String? = null)

@Composable
fun Conversation(navController: NavController, messages: List<Message>, modifier: Modifier = Modifier) {

    // Used so the conversation is always at the bottom
    val listState = rememberLazyListState()

    // Add the state to the LazyColumn
    LazyColumn(
        state = listState,
    ...
    // Scroll the screen to the bottom when the message list changes
